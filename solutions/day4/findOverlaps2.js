...
function checkOverlap(zoneA, zoneB) {
    if(parseInt(zoneB[1]) <= parseInt(zoneA[1]) && parseInt(zoneB[1]) >= parseInt(zoneA[0])) return true;
    if(parseInt(zoneB[0]) <= parseInt(zoneA[1]) && parseInt(zoneB[0]) >= parseInt(zoneA[0])) return true;
    else return false;
}
